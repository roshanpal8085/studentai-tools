...
export default function Game2048() {
  ...
  const restart = () => { setGrid(newGrid()); setScore(0); setWon(false); setOver(false); };
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-amber-900/20 to-slate-900 py-8 px-4">
        <div className="max-w-2xl mx-auto">
          ...
          <div className="flex justify-between items-center mb-4 gap-3">
            ...
            <button onClick={restart} className="bg-amber-500 hover:bg-amber-400 text-white font-bold px-5 py-3 rounded-xl transition-colors">New Game</button>
          </div>

          {/* Game Board */}
          <div
            className="bg-slate-700 rounded-2xl p-3 select-none touch-none"
            onTouchStart={onTouchStart}
            onTouchEnd={onTouchEnd}
          >
            <div className="grid grid-cols-4 gap-2">
              {grid.flat().map((val, i) => (
                <div key={i} className={`${colors[val] || 'bg-amber-300 text-white'} rounded-xl aspect-square flex items-center justify-center text-xl md:text-2xl font-extrabold transition-all duration-100`}>
                  {val || ''}
                </div>
              ))}
            </div>
          ...
          <div className="flex justify-center gap-4 mt-4">
            {['←', '↑', '↓', '→'].map((k, i) => (
              <button key={i} onClick={() => handleMove(['left','up','down','right'][i])}
                className="w-12 h-12 bg-slate-700 hover:bg-slate-600 text-white text-xl rounded-xl transition-colors font-bold">
                {k}
              </button>
            ))}
          </div>
          ...
          {/* Win / Over Overlay */}
          {(won || over) && (
            <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50" onClick={restart}>
