...
export default function FlappyBirdGame() {
  ...
  const flap = () => {
    ...
    const settings = getDifficultySettings(difficulty);
    if (stateRef.current) {
      stateRef.current.bird.vy = settings.jump;
      playSound('flap');
    }
  };
          ...
          <div className={status !== 'idle' ? "fixed inset-0 z-[100] bg-slate-900 overflow-y-auto lg:overflow-hidden" : "max-w-md mx-auto"}>
            <div className={status !== 'idle' ? "flex flex-col items-center justify-center h-screen py-4 px-4" : "relative flex flex-col bg-slate-800/50 rounded-3xl p-4 border border-white/10 shadow-2xl overflow-hidden min-h-[600px]"}>
              <div className={status !== 'idle' ? "w-full max-w-lg h-full max-h-[95vh] flex flex-col" : "w-full flex-1 flex flex-col"}>
                {status === 'idle' && (
                  <div className="absolute inset-0 z-10 bg-[#080c14]/90 backdrop-blur-2xl flex flex-col items-center justify-center p-6 text-center rounded-3xl border border-white/10 overflow-hidden">
                    {/* Animated Glow */}
                    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-64 h-64 bg-sky-500/20 rounded-full blur-[80px] animate-pulse pointer-events-none" />

                    <div className="relative z-20 flex flex-col items-center">
                      <div className="w-24 h-24 bg-gradient-to-br from-sky-400 to-blue-600 rounded-[2rem] flex items-center justify-center text-5xl shadow-2xl shadow-sky-500/20 mb-6 animate-bounce">
                        🐦
                      </div>
                      <h2 className="text-4xl sm:text-5xl font-black text-white mb-4 tracking-tight">Flappy Bird</h2>
                      <p className="text-slate-400 mb-10 max-w-sm text-sm sm:text-base font-medium leading-relaxed">
                        Tap to flap through the pipes. Test your timing and focus! Can you beat the high score?
                      </p>
              ...
              <div className="flex-1 min-h-0 flex items-center justify-center">
                <div className="relative rounded-2xl overflow-hidden border border-slate-700 cursor-pointer touch-none aspect-[2/3] max-h-full" onClick={flap} onTouchStart={e => { e.preventDefault(); flap(); }}>
                  <canvas ref={canvasRef} width={W} height={H} className="w-full h-full object-contain block shadow-2xl" />
                {status === 'idle' && (
